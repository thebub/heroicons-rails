...
namespace :heroicons do
  ...
  task "install" do
    ...
    if not File.exist?(Heroicons::Heroicon::METADATA_PATH)
      ...
      Rake::Task["heroicons:download"].invoke
    else
...
Rake::Task["assets:precompile"].enhance(["heroicons:install"])

if Rake::Task.task_defined?("test:prepare")
  Rake::Task["test:prepare"].enhance(["heroicons:install"])
